...
import type { CreateReproductiveEventDTO, ReproductiveAnimalMini } from '@/types/domain/reproduction.schema';
...
export default function ReproductionEventModal({ isOpen, onClose, onSuccess }: ReproductionEventModalProps) {
  ...
  const [eventType, setEventType] = useState<CreateReproductiveEventDTO['event_type']>('monta_natural');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (useExternalMale) {
        payload.male_animal_id = null;
        payload.male_external = maleExternal.trim() || null;
      } else {
        payload.male_external = null;
        payload.male_animal_id = maleId.trim() || null;
      }

      if (payload.male_animal_id && payload.male_external) {
        throw new Error('Use solo macho del inventario o solo texto externo, no ambos.');
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                value={eventType}
                onChange={(e) =>
                  setEventType(e.target.value as CreateReproductiveEventDTO['event_type'])
                }
                className="w-full bg-gray-50 border border-black/5 rounded-xl px-4 py-3 text-sm font-bold text-gray-700 outline-none focus:border-[var(--brand)]"
              >

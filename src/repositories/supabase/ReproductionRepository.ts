...
import { IReproductionRepository } from '../IReproductionRepository';
...
export class SupabaseReproductionRepository implements IReproductionRepository {
  ...
  async updateEvent(id: string, input: Partial<ReproductiveEvent>): Promise<ReproductiveEvent> {
    const { data, error } = await this.supabase
      .from('reproductive_events')
      .update(input)
      .eq('id', id)
  ...
  async listAnimalsBySex(sex: 'hembra' | 'macho'): Promise<any[]> {
    const { data, error } = await this.supabase
      .from('animals')
      .select('id, code, notes, species_id, reproductive_status')
      .eq('sex', sex)
      .eq('status', 'activo')
      .order('code');

    if (error) throw new Error(`Error al listar animales: ${error.message}`);
    return data || [];
  }
